...
public class GUI {
    ...
    public static void SimulationGUI(JFrame frame) {
        ...
        panel = new JPanel(new GridLayout(Controller.size, Controller.size));
        frame.setIconImage(logo.getImage());

        labelGrid = new JLabel[Controller.size][Controller.size];
        ...
        buttonBot.addActionListener(e -> {
            ...
            if (returnValue == JOptionPane.OK_OPTION) {
                try {
                    ...
                    Spawning.updateMap(Controller.size, Controller.size, Logic.npcList, Logic.weaponsList, Logic.medkitList);
                    display.setText("");
    ...
    public static void refreshGUIMap() {
        ...
        for (int i = 0; i < Logic.npcList.size(); i++) {
            labelGrid[Logic.npcList.get(i).posX][Logic.npcList.get(i).posY].setIcon(resizeImg(Logic.npcList.get(i).icon, cellSize, cellSize));
            labelGrid[Logic.npcList.get(i).posX][Logic.npcList.get(i).posY].setBorder(borderNPC);
        }
        for (int i = 0; i < Logic.weaponsList.size(); i++) {
            labelGrid[Logic.weaponsList.get(i).posX][Logic.weaponsList.get(i).posY].setIcon(resizeImg(Logic.weaponsList.get(i).icon, cellSize, cellSize));
            labelGrid[Logic.weaponsList.get(i).posX][Logic.weaponsList.get(i).posY].setBorder(borderWeapon);
        }
        for (int i = 0; i < Logic.medkitList.size(); i++) {
            labelGrid[Logic.medkitList.get(i)[0]][Logic.medkitList.get(i)[1]].setIcon(resizeImg(medkit, cellSize, cellSize));
            labelGrid[Logic.medkitList.get(i)[0]][Logic.medkitList.get(i)[1]].setBorder(borderMedkit);
        }
    }
